...
public class RotationPS3 : MonoBehaviour {
	...
	if ((Input.GetAxisRaw("4th axis")> 0.99f) && !isSpeeding){
		animation.Play("flap");
		isSpeeding = true;
		}
		
	if ((Input.GetAxisRaw("4th axis")< 0.9f) && isSpeeding) {
		StartCoroutine("Slow");
		isSpeeding = false;
		}
	...
	if ((Input.GetAxisRaw("X axis")> 0.3f)){
		EndPosition.eulerAngles = 	new Vector3(transform.rotation.eulerAngles.x, transform.rotation.eulerAngles.y, 27);
		transform.rotation = Quaternion.Lerp(StartPosition, EndPosition, rollSpeed);
		isMoving = true;
		}
		
	if ((Input.GetAxisRaw("X axis")> -0.3f) && (Input.GetAxisRaw("X axis")< 0.1f) ){
